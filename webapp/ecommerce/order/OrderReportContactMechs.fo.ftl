...
<#if orderHeader.getString("orderTypeId") == "PURCHASE_ORDER">
    <#if supplierGeneralContactMechValueMap??>
        ...
        <fo:block font-weight="bold" margin-top="0.25in">${uiLabelMap.OrderPurchasedFrom}:</fo:block>
...
<#list orderContactMechValueMaps as orderContactMechValueMap>
    ...
    <#if contactMech.contactMechTypeId == "POSTAL_ADDRESS">
        ...
        <fo:block font-weight="bold" margin-top="0.25in">${contactMechPurpose.get("description",locale)}:</fo:block>
        <fo:block text-indent="0.2in" >
            <#if postalAddress?has_content>
                <#if postalAddress.toName?has_content><fo:block>${postalAddress.toName!}</fo:block></#if>
                <#if postalAddress.attnName?has_content><fo:block>${postalAddress.attnName!}</fo:block></#if>
...
<#if orderPaymentPreferences?has_content>
    <fo:block font-weight="bold" margin-top="0.25in">${uiLabelMap.AccountingPaymentInformation}:</fo:block>
