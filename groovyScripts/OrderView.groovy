...
if (orderHeader) {
    ...
    if ("SALES_ORDER".equals(orderType)) {
        ...
        orderItemList.each { orderItem ->
            productPrice = from("ProductPrice").where("productId", orderItem.productId).queryFirst()
            if (productPrice) {
                BigDecimal priceWithoutTax = productPrice.priceWithoutTax
                orderItemSubTotal = priceWithoutTax.multiply(orderItem.quantity)
                BigDecimal priceTaxAmount = productPrice.taxAmount
                orderSalesTax = priceTaxAmount.multiply(orderItem.quantity)
                taxAmount = orderSalesTax.add(taxAmount)
            }
    ...
    isOrderFromRequest = "N";
    isEmptyOrderItemFromRequest = "N";
    requesterOrderRole = from("OrderRole").where("orderId", orderId, "roleTypeId", "BILL_TO_CUSTOMER").queryFirst();
    if (requesterOrderRole) {
        custRequestParty = from("CustRequestParty").where("partyId", requesterOrderRole.partyId, "roleTypeId", "REQ_REQUESTER").filterByDate().queryFirst();
        if (custRequestParty) {
            isOrderFromRequest = "Y";
        }
    }
    if (isOrderFromRequest.equals("Y")) {
        requesterOrderItems = from("OrderItem").where("orderId", orderId).queryList();
        if (!requesterOrderItems) {
            isEmptyOrderItemFromRequest = "Y";
        }
    }
    context.isOrderFromRequest = isOrderFromRequest;
    context.isEmptyOrderItemFromRequest = isEmptyOrderItemFromRequest;
    ...
    if ("PURCHASE_ORDER".equals(orderType)) {
        ...
        if (orderReadHelper.getBillToParty()) {
            ...
            Map ownedFacilities = new HashMap<String, Object>();
    ...
    if (productIds.size() > 0) {
        ...
        context.requiredProductQuantityMap = new HashMap<String, Object>();
        context.onOrderProductQuantityMap = new HashMap<String, Object>();
    }
}
...
if (orderHeader) {
    ...
    if ("PURCHASE_ORDER".equals(orderType)){
        shippingContactMechList = [];
        facility = delegator.findOne("Facility",["facilityId" : orderHeader.originFacilityId], false);
        if(facility){
            ownerPartyIds = EntityUtil.filterByDate(delegator.findByAnd("FacilityAndContactMech", [ownerPartyId : facility.ownerPartyId, contactMechTypeId : "POSTAL_ADDRESS"], null, false));
            ownerPartyIds.each { ownerPartyId ->
                contachMech = delegator.findOne("ContactMech",[contactMechId : ownerPartyId.contactMechId], false);
                shippingContactMechList.add(contachMech);
            }
        }
    } else {
    ...
    if (orderPaymentPreferences) {
        if (orderPaymentPreferences[0].paymentMethodTypeId == "GIFT_CARD") {
...
// This case comes when order's shipping amount is  more then or less than default percentage (defined in shipment.properties) of online UPS shipping amount.
    ...
    // get product reserve
    reservedNotAvailable = "N";
    orderHeader = delegator.findOne("OrderHeader", [orderId : orderId], false);
    reservations = orderHeader.getRelated("OrderItemShipGrpInvRes", null, null, false);
    reservations.each { reservation ->
        if (reservation.quantityNotAvailable) {
            reservedNotAvailable = "Y";
        }
    }
    context.reservedNotAvailable = reservedNotAvailable;

    // Find estimatedDeliveryDate purchase orders for this item.
    estimatedDeliveryDateList = [];
    for (productId in productIds) {
        estimatedDeliveryDateMap = [:];
        estimatedDeliveryDate = null;

        purchaseOrders = InventoryWorker.getOutstandingPurchaseOrders(productId, delegator);
        if (UtilValidate.isNotEmpty(purchaseOrders)){
            estimatedDeliveryDate = purchaseOrders[0].estimatedDeliveryDate;
        }

        estimatedDeliveryDateMap.productId = productId;
        estimatedDeliveryDateMap.estimatedDeliveryDate = estimatedDeliveryDate;
        estimatedDeliveryDateList.add(estimatedDeliveryDateMap);
    }
    context.estimatedDeliveryDateList = estimatedDeliveryDateList;
