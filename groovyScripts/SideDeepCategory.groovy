...
topCategoryList = [];
iter = topLevelList.iterator();
while (iter) {
  productCategory = iter.next();
  cm = productCategory.getRelated("ProductCategoryMember");
  if (cm.size() > 0) {
      topCategoryList.add(productCategory);
  }
}

request.setAttribute("topCategoryList", topCategoryList);
categoryList = request.getAttribute("topCategoryList");
if (categoryList) {
    catContentWrappers = [:];
